...
class Snippet(models.Model):
    ...
    title = models.CharField(
        max_length=100, blank=True, default="", db_comment="タイトル"
    )
    code = models.TextField(db_comment="コード")
    linenos = models.BooleanField(
        default=False, db_comment="行番号表示 true:表示/false:非表示"
    )
    language = models.CharField(
        choices=LANGUAGE_CHOICES, default="python", max_length=100, db_comment="言語名"
    )
